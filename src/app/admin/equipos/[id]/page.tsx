...
export default function AdminEquipoDetallePage() {
  ...
  const precioFicha =
    equipo.precio != null && Number(equipo.precio) > 0
      ? `S/ ${Number(equipo.precio).toLocaleString('es-PE', { maximumFractionDigits: 2 })}${equipo.unidad ? ` ${equipo.unidad}` : ''}`
      : null;
        ...
        <div className="flex items-center gap-2">
          {esModoLectura ? (
            ...
            <button
              ...
              className="px-4 py-2.5 bg-white border border-slate-200 text-slate-700 text-xs font-[700] rounded-[12px] hover:border-[#162B4D] transition-all inline-flex items-center gap-2"
            >
